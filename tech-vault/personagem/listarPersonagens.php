...
<link href="../css/bootstrap.min.css" rel="stylesheet">
  ...
  <div class="row">
    ...
    <div class="col-md-10 order-md-1 offset-md-1">
      <h4 class="mb-3">Lista de Personagens</h4>
      <table class="table">
        ...
        <tbody>
          <?php
            ...
            $result = $conexao->query("SELECT p.id, g.name AS game_name, p.name AS per_name FROM personagem p INNER JOIN game g ON g.id=p.id_game");

            if ($result){
              foreach($result as $row){
                echo '<tr> 
                        <th scope="row">'. $row['id'] .'</th>
                        <td><a href="../combo/listarCombos.php?idPersonagem='.$row['id'].'">'. $row['per_name'] .'</a></td>
                        <td>'. $row['game_name'] . '</td>
                        <td><a href="personagem.php?acao=excluir&id=' . $row['id'] . '">Excluir</a></td>
                     </tr>';
              }
            }
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="btn-group">
          <a href="formCadastroPersonagens.php" class="btn btn-sm btn-outline-secondary">Cadastrar Novo</a>
        </div>
      </div>
    </div>
  </div>

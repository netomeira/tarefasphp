...
<link href="../css/bootstrap.min.css" rel="stylesheet">
  ...
  <div class="row">
    ...
    <div class="col-md-10 order-md-1 offset-md-1">
      <h4 class="mb-3">Lista de Combos</h4>
      <table class="table">
        ...
        <tbody>
          <?php
            ...
            $query = "SELECT c.id, p.name, c.description, c.instructions, c.dmg_total, c.custo FROM combo c INNER JOIN personagem p ON p.id = c.id_personagem";
            
            if (isset($_GET['idPersonagem'])){
              $query = $query . " WHERE c.id_personagem=" . $_GET['idPersonagem'];
            }
            ...
            if ($result){
              foreach($result as $row){
                echo '<tr> 
                        ...
                        <td>'. $row['dmg_total'] . '</td>
                        <td>'. $row['custo'] . '</td>
                        <td><a href="combo.php?acao=excluir&id=' . $row['id'] . '">Excluir</a></td>
                     </tr>';
              }
            }
      ...
      <div class="d-flex justify-content-between align-items-center">
        <div class="btn-group">
          <a href="formCadastroCombos.php" class="btn btn-sm btn-outline-secondary">Cadastrar Novo</a>
        </div>
      </div>
    </div>
  </div>
